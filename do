...
macOSURL=$(curl -s https://raw.githubusercontent.com/hea07/easyErase_12/main/latest)
	...
	if [[ -f ${CACHEDISK}${PKG} ]]; then      
	    	...
	    	echo -e "${GREEN}[INFO]:${NC} Install macOS Monterey.app from cache disk.."
	    	...
	    	checkInstallerVersion=$(defaults read /Applications/Install\ macOS\ Monterey.app/Contents/Info.plist DTPlatformVersion)
		    ...
		    if [ "$ServerVersion" = "$checkInstallerVersion" ]; then
	    		...
	    		SilentAttach=$(hdiutil attach -quiet -noverify /Applications/Install\ macOS\ Monterey.app/Contents/SharedSupport/SharedSupport.dmg)
	    		# 
			...
			echo -e "${GREEN}[INFO]:${NC} Installing application Install macOS Monterey.app"
			installer -pkg "${TMPDIR}${PKG}" -target /  >/dev/null
	fi
...
if [[ "$os_ver" == 10.* ]]; then
/Applications/Install\ macOS\ Monterey.app/Contents/Resources/startosinstall --agreetolicense --eraseinstall --forcequitapps --newvolumename "Macintosh HD" > /dev/null 2>&1  
elif [[ "$os_ver" == 11.* ]]; then
/Applications/Install\ macOS\ Monterey.app/Contents/Resources/startosinstall --agreetolicense --eraseinstall --newvolumename "Macintosh HD" --forcequitapps --passprompt
elif [[ "$os_ver" == 12.* ]]; then
/Applications/Install\ macOS\ Monterey.app/Contents/Resources/startosinstall --agreetolicense --eraseinstall --newvolumename "Macintosh HD" --forcequitapps --passprompt
else
...
echo -e "${GREEN}[INFO]:${NC} Start macOS Monterey recover for M1.. "
...
cp -R 'Source/Payload/Applications/Install macOS Monterey.app' "${dstDisk}"/private/tmp/
mkdir -p Install\ macOS\ Monterey.app/Contents/SharedSupport
/bin/chmod 0755 Install\ macOS\ Monterey.app/Contents/SharedSupport
mv InstallAssistant.pkg Install\ macOS\ Monterey.app/Contents/SharedSupport/SharedSupport.dmg
/bin/chmod 0644 Install\ macOS\ Monterey.app/Contents/SharedSupport/SharedSupport.dmg
/usr/sbin/chown -R root:wheel Install\ macOS\ Monterey.app/Contents/SharedSupport/SharedSupport.dmg
/usr/bin/chflags -h norestricted Install\ macOS\ Monterey.app/Contents/SharedSupport/SharedSupport.dmg
rm -rf Source
...
"${dstDisk}"/private/tmp/Install\ macOS\ Monterey.app/Contents/MacOS/InstallAssistant_springboard >/dev/null
...
function runRecoveryModeIntel () {

echo -e "\n${RED}[WARNING]:${NC} This script remove all data on your computer!"
echo -e "${RED}[WARNING]:${NC} Press CTRL-Z to cancel this process."
echo -e "${GREEN}[INFO]:${NC} Start macOS Monterey recover for Intel machines.. "
...
if [[ ! -d "${dstDisk}/private/tmp/Source/Payload/Applications/Install macOS Monterey.app" ]]; then   
...
externalDisk=$(diskutil list | grep external | awk -F " " {'print $1'})
diskutil umountDisk $externalDisk
"${dstDisk}"/private/tmp/Install\ macOS\ Monterey.app/Contents/Resources/startosinstall --agreetolicense --volume "${dstDisk}" 
#> /dev/null 2>&1  
